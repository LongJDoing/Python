...
def L2region(v1,v2):
    ...
    v1_f = v1.flatten()
    v2_f = v2.flatten()
    vtp = float(sum(abs(v1_f-v2_f)))/(sum(abs(v1_f)) + sum(abs(v2_f)))
    mint = vtp
    ...
    for i in range(3):
        for j in range(3):
            ...
            tmp = float(sum(abs(v1_f-v2_f)))/(sum(abs(v1_f)) + sum(abs(v2_f)))
            if mint > tmp:
                mint = tmp
    '''
    return mint
       
def hcluster(features, B_ool=False):
...
#    printnode[1].get_cluster_elements()
    while len(node)>1:
        closest = float('Inf')
